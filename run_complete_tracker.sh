...
check_dependencies() {
    ...
    python3 -c "import requests, beautifulsoup4, pandas, dotenv, firebase_admin" 2>/dev/null || {
...
run_scraper() {
    ...
    if [ $SCRAPER_EXIT_CODE -eq 0 ]; then
        ...
        # Check if Firebase integration is working
        if echo "$SCRAPER_OUTPUT" | grep -q "🔥 Saved.*products to Firebase"; then
            print_status "Firebase integration working - data saved to cloud"
        else
            print_warning "Firebase integration may not be working - check logs"
        fi
    else
...
run_analysis() {
    ...
    if python3 -c "import firebase_admin" 2>/dev/null; then
        print_info "Running Firebase analytics..."
        FIREBASE_ANALYSIS_OUTPUT=$(python3 scripts/firebase_analytics.py --report-only 2>&1)
        FIREBASE_EXIT_CODE=$?
        
        if [ $FIREBASE_EXIT_CODE -eq 0 ]; then
            print_status "Firebase analytics completed"
            echo "$FIREBASE_ANALYSIS_OUTPUT" >> logs/firebase_analytics_$(date +%Y%m%d).log
        else
            print_warning "Firebase analytics failed, trying CSV analysis"
        fi
    fi
    
    # Run the traditional CSV analysis script
    ANALYSIS_OUTPUT=$(python3 scripts/analyze_data.py 2>&1)
    ANALYSIS_EXIT_CODE=$?
    
    if [ $ANALYSIS_EXIT_CODE -eq 0 ]; then
        print_status "CSV analysis completed successfully"
...
try:
    ...
    print(f'  Firebase not available: {str(e)[:50]}...')
" 2>/dev/null || echo "  Firebase integration not available")
...
- Data Directory: $(if [ -d "data/processed" ]; then echo "Exists"; else echo "Missing"; fi)
- Logs Directory: $(if [ -d "logs" ]; then echo "Exists"; else echo "Missing"; fi)
- Firebase Integration: $(python3 -c "import firebase_admin; print('Available')" 2>/dev/null && echo "Available" || echo "Not Available")
